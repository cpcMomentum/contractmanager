 ...
 *
 * „Was ist neu?"-Fenster (#415): Das Fenster darf nur erscheinen, wenn es
...
vi.mock('@nextcloud/vue/components/NcModal', () => ({
	default: {
		...
		// Nextcloud-Leiste ueberdeckt (#415).
		props: ['labelId', 'name'],
		emits: ['close'],
...
const eintrag = (teil: Partial<WhatsNewEntry>): WhatsNewEntry => ({
	title: 'Kategorien per Ziehen',
	text: 'Vertrag auf eine Kategorie ziehen.',
	icon: 'tag',
	where: '',
	adminOnly: false,
	plus: false,
...
describe('WhatsNewDialog', () => {
	...
	it('zeigt Titel und Text der Eintraege', async () => {
		getWhatsNew.mockResolvedValue(payload([
			eintrag({ title: 'Kategorien per Ziehen', text: 'Vertrag auf eine Kategorie ziehen.', plus: false }),
			eintrag({ title: 'Sicherungsordner', text: 'Der Pfad bleibt stehen.', plus: false }),
		]))
		...
		expect(wrapper.text()).toContain('Kategorien per Ziehen')
		expect(wrapper.text()).toContain('Der Pfad bleibt stehen.')
	...
	it('zeigt Badge und Link nur bei WerkPlus-Eintraegen', async () => {
		getWhatsNew.mockResolvedValue(payload([
			...
			eintrag({ title: 'Sicherungsordner', text: 'Vertrag auf eine Kategorie ziehen.', plus: false }),
		]))
	...
	it('zeigt die Fundort-Zeile nur, wenn ein Ort angegeben ist', async () => {
		getWhatsNew.mockResolvedValue(payload([
			eintrag({ title: 'Mit Ort', where: 'Verträge' }),
		...
		expect(zeilen[0].text()).toContain('Verträge')
	})
