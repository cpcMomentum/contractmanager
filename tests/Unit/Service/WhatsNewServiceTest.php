...
/**
 * Anzeige-Logik des „Was ist neu?"-Fensters (#415, Rollout aus dem Pilot RechnungsWerk#308).
 *
...
class WhatsNewServiceTest extends TestCase {
	...
	private const BEKANNTE_SYMBOLE = [
		'account-group', 'archive', 'bell-ring', 'calendar', 'chart-bar',
		'cog', 'email', 'file-document', 'folder', 'magnify', 'star',
		'tag', 'translate',
	];
	...
	protected function setUp(): void {
		parent::setUp();
		$this->appDir = sys_get_temp_dir() . '/cm-whatsnew-' . bin2hex(random_bytes(6));
	...
	private function catalogueFixture(): array {
		return [
			...
			'1.8.0' => [
				[
					'title' => ['de' => 'Kategorien per Ziehen', 'en' => 'Categories by dragging'],
					'text' => ['de' => 'Vertrag auf eine Kategorie ziehen', 'en' => 'Drag a contract onto a category'],
					'icon' => 'tag',
					'where' => ['de' => 'Verträge', 'en' => 'Contracts'],
					'adminOnly' => false,
					'plus' => false,
				],
				[
					'title' => ['de' => 'Sicherungsordner', 'en' => 'Backup folder'],
	...
	public function testBestandsinstallationSiehtDasEinfuehrungsFenster(): void {
		...
		$this->writeCatalogue($this->catalogueFixture());
		$written = [];
		$service = $this->buildService('1.8.0', [], $written, 'de', '1.8.0');
	...
	public function testNotizGiltNurFuerDasEinfuehrendeRelease(): void {
		// Die Notiz stammt von 1.8.0, installiert ist inzwischen 1.8.1. Ein
		// Konto, das erst jetzt dazukommt, ist wirklich neu.
		$this->writeCatalogue(['1.8.0' => $this->catalogueFixture()['1.8.0'], '1.8.1' => [[
			'title' => ['de' => 'Spaeter', 'en' => 'Later'],
			'text' => ['de' => 'Text', 'en' => 'Text'],
		]]]);
		$written = [];
		$service = $this->buildService('1.8.1', [], $written, 'de', '1.8.0');

		$result = $service->getPending('alice');

		self::assertSame([], $result['entries'], 'Spaetere Neuzugaenge werden nicht begruesst');
		self::assertSame('1.8.1', $written['alice/' . WhatsNewService::KEY_LAST_SEEN] ?? null);
	}

	public function testOhneNotizBleibtEsBeimErstkontakt(): void {
		$this->writeCatalogue($this->catalogueFixture());
		$written = [];
		$service = $this->buildService('1.8.0', [], $written, 'de', '');
	...
	public function testNeueVersionLiefertDerenEintraege(): void {
		$this->writeCatalogue($this->catalogueFixture());
		$written = [];
		$service = $this->buildService('1.8.0', ['alice/' . WhatsNewService::KEY_LAST_SEEN => '1.7.1'], $written);

		$result = $service->getPending('alice');

		self::assertSame('1.8.0', $result['version']);
		self::assertCount(2, $result['entries']);
		self::assertSame('Kategorien per Ziehen', $result['entries'][0]['title']);
		self::assertSame('Vertrag auf eine Kategorie ziehen', $result['entries'][0]['text']);
		self::assertFalse($result['entries'][0]['plus']);
		self::assertSame('tag', $result['entries'][0]['icon']);
		self::assertSame('Verträge', $result['entries'][0]['where']);
	...
	public function testVersionssprungKumuliertNicht(): void {
		$this->writeCatalogue($this->catalogueFixture());
		$written = [];
		$service = $this->buildService('1.8.0', ['alice/' . WhatsNewService::KEY_LAST_SEEN => '1.5.0'], $written);
	...
	public function testNichtsNeuesSchreibtDieMarkeStillFort(): void {
		$this->writeCatalogue($this->catalogueFixture());
		$written = [];
		$service = $this->buildService('1.8.1', ['alice/' . WhatsNewService::KEY_LAST_SEEN => '1.8.0'], $written);
	...
	public function testEintraegeAusDerZukunftBleibenAus(): void {
		$this->writeCatalogue($this->catalogueFixture());
		$written = [];
		// Installiert ist 1.7.1, die Datei kennt schon 1.8.0.
		$service = $this->buildService('1.7.1', ['alice/' . WhatsNewService::KEY_LAST_SEEN => '1.6.0'], $written);

		$result = $service->getPending('alice');

		self::assertSame('1.7.0', $result['version'], 'Nur was auch installiert ist');
	...
	public function testEnglischeNutzerBekommenDenEnglischenText(): void {
		$this->writeCatalogue($this->catalogueFixture());
		$written = [];
		$service = $this->buildService('1.8.0', ['alice/' . WhatsNewService::KEY_LAST_SEEN => '1.7.1'], $written, 'en');

		$result = $service->getPending('alice');

		self::assertSame('Categories by dragging', $result['entries'][0]['title']);
	}
	...
	public function testFundortFolgtDerSprache(): void {
		...
		self::assertSame('Contracts', $result['entries'][0]['where']);
	}

	public function testEintragOhneSymbolUndFundortBleibtGueltig(): void {
		$this->writeCatalogue(['1.8.0' => [[
	...
	public function testRegionalcodeFaelltAufDieBasisspracheZurueck(): void {
		$this->writeCatalogue($this->catalogueFixture());
		$written = [];
		$service = $this->buildService('1.8.0', ['alice/' . WhatsNewService::KEY_LAST_SEEN => '1.7.1'], $written, 'de_DE');
	...
	public function testUnbekannteSpracheFaelltAufEnglischZurueck(): void {
		$this->writeCatalogue($this->catalogueFixture());
		$written = [];
		$service = $this->buildService('1.8.0', ['alice/' . WhatsNewService::KEY_LAST_SEEN => '1.7.1'], $written, 'fr');
	...
	public function testMarkenZweierNutzerBleibenGetrennt(): void {
		$this->writeCatalogue($this->catalogueFixture());
		$written = [];
		// Alice hat das Fenster schon gesehen, Bob steht noch auf 1.7.1.
		$service = $this->buildService('1.8.0', [
			'alice/' . WhatsNewService::KEY_LAST_SEEN => '1.8.0',
			'bob/' . WhatsNewService::KEY_LAST_SEEN => '1.7.1',
		], $written);
		...
		self::assertSame('1.8.0', $written['bob/' . WhatsNewService::KEY_LAST_SEEN]);
	}
	...
	public function testAppIdBleibtDerAblageortDerMarke(): void {
		...
		self::assertSame('contractmanager', Application::APP_ID);
	}
}
